...
另外，测试文件中使用了一个 `MaxReplicas` 的变量，对于只有单个 `Peer` 的 `Region` 而言，当 `MaxReplicas = 1` 时允许迁移，而当 `MaxReplicas = 3` 时禁止迁移，所以我认为挑选迁出 `Region` 时还应满足条件 `Region.GetPeerCounts() >= MaxReplicas`。
