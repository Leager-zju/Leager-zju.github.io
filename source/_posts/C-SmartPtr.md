...
既然允许共享，那就存在多个 `shared_ptr` 使用同一片内存，故不能直接析构时释放，为了应对这一问题，类内维护了一个**引用计数器**，每当以拷贝的形式新建了一个 `shared_ptr`，则指向同一片内存的所有 `shared_ptr` 的引用计数都会加一，每次析构的时候引用计数减一，在最后一个 `shared_ptr` 析构的时候（也就是引用计数归零时），对应的内存才会释放。
