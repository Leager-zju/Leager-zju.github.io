...
### 网格体操作(Mesh Operation)

有以下三种操作：

1. **网格细分(Mesh Subdivision)**：用更多的三角形得到更光滑的表面，如下面图 2 到图 3；
2. **网格简化(Mesh Simplification)**：用更少的三角形来节约存储，如下面图 2 到图 4；
3. **网格正规化(Mesh Regularization)**：尽可能将三角形转换/近似为正三角形，防止三角形出现特别尖长的情况，如下面图 2 到图 5；
