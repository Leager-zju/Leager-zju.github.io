...
进程会在两个时候被创建，一个是 `userinit()`(`kernal/proc.c`)时，一个是调用 `fork()` 时。进程的创建会调用 `kernal/proc.c` 中的 `allocproc()`，然后通过 `proc_pagetable()` 来初始化页表。

我们要做的很简单。

第一步，在页表初始化时加入一个 `{USYSCALL => PHYPAGE}` 的 PTE。`USYSCALL` 在 `kernal/memlayout.h` 中定义（就是 `TRAPFRAME` 的前一页），而 `PHYPAGE` 则需要我们调用 `kalloc()`(`kernal/kalloc.c`) 进行空闲页分配。
...
添加 PTE 的操作很简单，页表相关操作都在 `kernal/vm.c` 中，我们需要关注 `mempages()` 函数，该函数就是为指定页表添加 PTE。唯一需要关注的就是符号控制位，根据我们的需求，该页面为只读型，而且能被用户层函数访问，那么只赋予 `PTE_R|PTE_U` 即可。有效位会在 `mappages()` 内部添加，无需操心。
...
当然，不仅要考虑分配，还要考虑释放的操作。如果到这一步就急匆匆地 `make qemu; ./pgtbltest`，那你马上就会得到一个 `panic: freewalk leaf`。这是因为我们没有在清空页表时清空我们已分配的 `PHYPAGE`，`proc_freepagetable()`(`kernal/proc.c`) 的注释讲的很清楚：**Free a process's page table, and free the physical memory it refers to.** 并且它也给了提示：要调用 `uvmunmap()`。所以新增一行即可，实际的释放操作会在 `uvmunmap()` 中完成：
...
现在我们需要按照给定格式，在 `kernal/exec.c:119` 插入 `if(p->pid==1) vmprint(p->pagetable)` 来输出初始进程的页表，以可视化并有助于 debug。

参考 `kernal/vm.c` 中的 `freewalk()`，我们主要关注的是它以怎样的方式遍历页目录结构——获取表项，得到物理地址，根据状态位判断这一物理地址是页表还是数据页，决定是否需要进一步深入。

> 在 xv6 中，每个页表项的低位为状态位，如下图所示：
> 
> <img src="1.png"/>
> 
> `kernal/riscv.h` 中已经为我们定义好了若干状态位，直接使用即可。
...
那么先在 `kernal/def.h` 中添加函数签名，然后修改 `kernal/exec.c`，最后就是在 `kernal/vm.c` 下加入两个函数的代码了。
...
既然如此，那每个页表项就需要一个“访问位”咯，根据上面那张图，我们可以在 `kernal/riscv.h` 中添加一行 `#define PTE_A (1L << 6)`，方便进行位运算。

lab 已经贴心地为我们做好了所有准备工作，包括修改 `usys.pl`、注册系统调用等，所以我们只需要在 `kernal/sysproc.c` 中实现 `sys_pgaccess()` 函数即可。
...
那么如何根据页表找到最终的页表项呢？`kernal/vm.c` 中为我们提供了 `walk()` 的 API，它根据虚拟地址，返回指向数据页的那个页表项的指针。还有一个叫 `walkaddr()` 的，是直接返回物理页的起始地址，后面有用，这里暂时用不到。
